...
#ifndef PCC_REAL
#define PCC_REAL double // or float
#endif
...
namespace detail
{
    ...
    template <class T>
    inline constexpr T two_pi_v = T(2) * std::numbers::pi_v<T>;

    template <class T>
    inline constexpr T one_over_two = T(0.5);
    ...
    template <class T>
    PCC_ALWAYS_INLINE T wrap_0_2pi(T a) noexcept
    {
        T t = std::fmod(a, two_pi_v<T>);
        if (t < T(0))
            t += two_pi_v<T>;
        if (t >= two_pi_v<T>)
            t -= two_pi_v<T>;
    ...
    PCC_ALWAYS_INLINE void A_B_scalar_stable_ab(T t, T &A, T &B) noexcept
    {
        ...
        if (at < small) [[likely]]
        {
            ...
            A = std::fma(T(-1.0 / 40320.0), t7,
                         std::fma(T(1.0 / 720.0), t5,
                                  std::fma(T(-1.0 / 24.0), t3, T(0.5) * t)));
    ...
    template <class T>
    PCC_ALWAYS_INLINE void A_B_scalar_stable(T t, T &A, T &B, T &Ap, T &Bp) noexcept
    {
        const T at = std::abs(t);
        const T small = T(8) * std::cbrt(eps_v<T>);
        ...
        if (at < small) [[likely]]
        {
            ...
            Ap = std::fma(T(-1.0 / 5760.0), t6,
                          std::fma(T(1.0 / 144.0), t4,
                                   std::fma(T(-1.0 / 8.0), t2, T(0.5))));
...
struct [[nodiscard]] Mat3
{
    ...
    {
        return {
            std::fma(a02, v.z, std::fma(a01, v.y, a00 * v.x)),
    ...
    PCC_ALWAYS_INLINE Mat3 mul(const Mat3 &B) const noexcept
    {
        const Mat3 &A = *this;
        return {
            std::fma(A.a01, B.a10, std::fma(A.a02, B.a20, A.a00 * B.a00)),
...
PCC_ALWAYS_INLINE Mat3<T> RzRyRz_compact(T phi, T theta) noexcept
{
    ...
    return {
        cphi2 * ctheta + sphi2, cs * (ctheta - T(1)), cphi * stheta,
        cs * (ctheta - T(1)), sphi2 * ctheta + cphi2, sphi * stheta,
        -cphi * stheta, -sphi * stheta, ctheta};
}

template <class T>
PCC_ALWAYS_INLINE Mat3<T> RzRyRz(T phi, T theta) noexcept { return RzRyRz_compact<T>(phi, theta); }
...
static inline Real wrap_0_2pi_R(Real a)
{
    return detail::wrap_0_2pi(a);
}
...
// ------------------------------ geometry kernels -----------------------------------------
template <class T>
PCC_ALWAYS_INLINE void inner_angles_from_orientation(
    const Mat3<T> &R1, const Vec3<T> &n_star, T alpha,
    T &phi2_wrapped, T &theta2, Vec3<T> &z2_world, Mat3<T> &R2) noexcept
{
    // n' = R1^T * n*
    const Vec3<T> nprime{
    ...
    const T cph = std::cos(phi2), sph = std::sin(phi2);
...
static PCC_HOT PCC_FLATTEN Cand evaluate_once(
    ...
    const SolverConst &sc) noexcept
{
    ...
    Real phi2, theta2;
    inner_angles_from_orientation(R1, n_star, alpha, phi2, theta2, z2w, R2);
    ...
    {
        if (!(seg2.phi_min - Real(1e-9) <= phi2 && phi2 <= seg2.phi_max + Real(1e-9)))
        {
            phi2 = project_angle_to_interval(phi2, seg2.phi_min, seg2.phi_max);
        }
    }
    R2 = RzRyRz(phi2, theta2);
    ...
    const Vec3<Real> dP_local{
        R1.a00 * dP.x + R1.a10 * dP.y + R1.a20 * dP.z,
    ...
    const Vec3<Real> q_no_L2p{
        Rz2_v2.x + seg2.rigid_tip_length * R2_ez.x,
    ...
    const Vec3<Real> b_world = R1R2.apply(b0).normalized();
    ...
    const Vec3<Real> diff{end_p.x - P_star.x, end_p.y - P_star.y, end_p.z - P_star.z};
    const Real pos_err = diff.norm();
    if (pos_err > sc.pos_tol + Real(1e-9) || cb < cos_bevel_tol) [[unlikely]]
        return out;

    const Vec3<Real> ezw = R1R2.apply({Real(0), Real(0), Real(1)}).normalized();
    const Real cos_axis = std::clamp(ezw.dot(n_star), Real(-1), Real(1));
    const Real ang_err_deg = std::abs(std::acos(cos_axis) * Real(180) / std::numbers::pi_v<Real> - sc.angle_target_deg);
...
static std::pair<Real, Cand> brent_refine_phi_native(
    ...
    const SegmentBounds &seg1, const SegmentBounds &seg2, const SolverConst &sc)
{
    ...
    for (int it = 0; it < maxiter; ++it)
    {
        ...
        if (std::abs(e) > tol1)
        {
            ...
            if (std::abs(p) < std::abs(Real(0.5) * q * e) && p > q * (a - x) && p < q * (b - x))
            {
                d = p / q;
                const Real u = x + d;
    ...
    Real &best_th, Real &best_ph)
{
    ...
    auto residual = [&](Real th, Real ph, Vec3<Real> &endp, Vec3<Real> &bev) -> std::array<Real, 6>
    {
        ...
        endp = c.end_p;
        bev = c.b_world;
        const Vec3<Real> rp{P_star.x - endp.x, P_star.y - endp.y, P_star.z - endp.z};
    ...
    for (int it = 0; it < iters; ++it)
    {
        ...
        Real ph_try = ph + s1;
        ph_try = seg1.has_phi_bounds ? project_angle_to_interval(ph_try, seg1.phi_min, seg1.phi_max)
                                     : wrap_0_2pi_R(ph_try);
    ...
    const auto cfin = evaluate_once(best_th, best_ph, P_star, n_star, seg1, seg2, sc);
    ...
    int k_keep = 6)
{
...

    {
        ...
        auto worker = [&](int begin, int end)
        {
            ...
            {
                auto cand = evaluate_once(theta1, ph(i), P_star, n_star, seg1, seg2, sc);
                if (!cand.ok)
                    continue;
                const Real J = cand.pos_err + Real(1e-3) * cand.ang_err_deg;
...
#pragma omp parallel
        ...
        for (auto &vv : buckets)
        {
            merged.insert(merged.end(), std::make_move_iterator(vv.begin()), std::make_move_iterator(vv.end()));
        }
#else
    ...
    const SegmentBounds &seg1, const SegmentBounds &seg2, const SolverConst &sc)
{
    ...
    auto [phi_best, cand] = brent_refine_phi_native(
        theta1, phi_seed, halfspan_deg, maxiter, P_star, n_star, seg1, seg2, sc);
    ...
    const Vec3<Real> q_local{
        Rz2_v2.x + seg2.rigid_tip_length * R2_ez.x,
        Rz2_v2.y + seg2.rigid_tip_length * R2_ez.y,
        Rz2_v2.z + seg2.rigid_tip_length * R2_ez.z + cand.L2p};
    const Vec3<Real> R1_q = R1.apply(q_local);
    const Vec3<Real> end_p_no_d{p1.x + R1_q.x, p1.y + R1_q.y, p1.z + R1_q.z};
    ...
    const SegmentBounds &seg1, const SegmentBounds &seg2, const SolverConst &sc)
{
    ...
    const auto cfin = evaluate_once(th_fin, ph_fin, P_star, n_star, seg1, seg2, sc);
    if (!cfin.ok)
    {
        py::dict d;
        d["theta1"] = th_fin;
    ...
    const Vec3<Real> q_local{
        Rz2_v2.x + seg2.rigid_tip_length * R2_ez.x,
        Rz2_v2.y + seg2.rigid_tip_length * R2_ez.y,
        Rz2_v2.z + seg2.rigid_tip_length * R2_ez.z + cfin.L2p};
    const Vec3<Real> R1_q = R1.apply(q_local);
    const Vec3<Real> end_p_no_d{p1.x + R1_q.x, p1.y + R1_q.y, p1.z + R1_q.z};
...
PYBIND11_MODULE(_core, m)
{
    ...
    m.def("phi_scan", &phi_scan,
          py::arg("theta1"), py::arg("phi_list"),
          py::arg("P_star"), py::arg("n_star"),
          py::arg("seg1"), py::arg("seg2"), py::arg("sc"),
          py::arg("k_keep") = 6);

    m.def("evaluate_once", &evaluate_once_py,
          py::arg("theta1"), py::arg("phi1"),
          py::arg("P_star"), py::arg("n_star"),
          py::arg("seg1"), py::arg("seg2"), py::arg("sc"));

    m.def("brent_refine_phi", &brent_refine_phi_py,
          py::arg("theta1"), py::arg("phi_seed"),
          py::arg("halfspan_deg"), py::arg("maxiter"),
          py::arg("P_star"), py::arg("n_star"),
          py::arg("seg1"), py::arg("seg2"), py::arg("sc"));

    m.def("lm_polish", &lm_polish_py,
          py::arg("theta1"), py::arg("phi1"), py::arg("iters"),
          py::arg("P_star"), py::arg("n_star"),
          py::arg("seg1"), py::arg("seg2"), py::arg("sc"));
}
